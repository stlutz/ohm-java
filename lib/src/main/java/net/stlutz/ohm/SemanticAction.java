...
class SemanticAction {
	...
	static SemanticAction fromMethod(String actionName, Method method) {
		...
		if (method.isVarArgs() && parameterTypes.length >= 2) {
			...
			throw new OhmException(
					"Action method '%s' is varargs but has more than one parameter".formatted(method.getName()));
	...
	void validateAgainstGrammar(Grammar grammar) {
		...
		int expectedArity = name.equals(SpecialActionNames.terminal) ? 0 : rule.getArity();
		int actualArity = method.getParameterCount();
		if (expectedArity != actualArity) {
			throw new OhmException("Rule '%s' expects an action with '%d' arguments, but got '%d'".formatted(name,
	 ...
	 * <li>= 0, if {@code actionA} and {@code actionB} are declared in the same
	 * class,</li>
