...
public class Grammar {
	...
	Grammar(String name, Grammar superGrammar, Map<String, Rule> rules, String defaultStartRule, boolean isBuiltIn) {
		super();
		this.name = name;
	...
	public void setDefaultStartRule(String defaultStartRule) {
		if (defaultStartRule == null) {
			...
			throw new OhmException(
					"Invalid default start rule: '%s' is not a rule in grammar '%s'".formatted(defaultStartRule, name));
		}
		this.defaultStartRule = defaultStartRule;
	}
	...
	static Namespace buildDefaultNamespace() {
		...
		grammar.newRule("caseInsensitive").formals("str").body(new CaseInsensitiveTerminal(new Param(0)));
		...
		grammar.newRule("unicodeLtmo").description("a Unicode character in Lt, Lm, or Lo").body(unicodeChar("Ltmo"));
		...
		grammar.newRule("alnum").description("an alpha-numeric character").body(alt(apply("letter"), apply("digit")));
		...
		grammar.newRule("ListOf").formals("elem", "sep")
				.body(alt(apply("NonemptyListOf", param(0), param(1)), apply("EmptyListOf", param(0), param(1))));
		grammar.newRule("NonemptyListOf").formals("elem", "sep").body(seq(param(0), star(seq(param(1), param(0)))));
	...
	static Grammar buildOhmGrammar() {
		...
		grammar.newRule("Grammar").body(
				seq(apply("ident"), opt(apply("SuperGrammar")), terminal("{"), star(apply("Rule")), terminal("}")));
		grammar.newRule("SuperGrammar").body(seq(terminal("<:"), apply("ident")));
		// TODO: handle inline rule declarations explicitly?
		grammar.newRule("Rule").body(alt(apply("Rule_define"), apply("Rule_override"), apply("Rule_extend")));
		grammar.newRule("Rule_define").body(
				seq(apply("ident"), opt(apply("Formals")), opt(apply("ruleDescr")), terminal("="), apply("RuleBody")));
		grammar.newRule("Rule_override")
				.body(seq(apply("ident"), opt(apply("Formals")), terminal(":="), apply("OverrideRuleBody")));
		grammar.newRule("Rule_extend")
				.body(seq(apply("ident"), opt(apply("Formals")), terminal("+="), apply("RuleBody")));
		grammar.newRule("RuleBody")
				.body(seq(opt(terminal("|")), apply("NonEmptyListOf", apply("TopLevelTerm"), terminal("|"))));
		...
		grammar.newRule("OverrideRuleBody")
				.body(seq(opt(terminal("|")), apply("NonEmptyListOf", apply("OverrideTopLevelTerm"), terminal("|"))));
		...
		grammar.newRule("Params").body(seq(terminal("<"), apply("ListOf", apply("Seq"), terminal(",")), terminal(">")));
		grammar.newRule("Alt").body(apply("NonEmptyListOf", apply("Seq"), terminal("|")));
		grammar.newRule("Seq").body(star(apply("Iter")));
		grammar.newRule("Iter").body(alt(apply("Pred_star"), apply("Pred_plus"), apply("Pred_opt"), apply("Pred")));
		...
		grammar.newRule("Base")
				.body(alt(apply("Base_application"), apply("Base_range"), apply("Base_terminal"), apply("Base_paren")));
		...
		grammar.newRule("Base_range").body(seq(apply("oneCharTerminal"), terminal(".."), apply("oneCharTerminal")));
		...
		grammar.newRule("caseName")
				.body(seq(terminal("--"), star(seq(not(terminal("\n")), apply("space"))), apply("name"),
						star(seq(not(terminal("\n")), apply("space"))), alt(terminal("\n"), lookahead(terminal("}")))));
		grammar.newRule("name").description("a name").body(seq(apply("nameFirst"), star(apply("nameRest"))));
		...
		grammar.newRule("terminal").body(seq(terminal("\""), star(apply("terminalChar")), terminal("\"")));
		grammar.newRule("oneCharTerminal").body(seq(terminal("\""), apply("terminalChar"), terminal("\"")));
		...
		grammar.newRule("escapeChar").description("an escape sequence")
				.body(alt(apply("escapeChar_backslash"), apply("escapeChar_doubleQuote"),
						apply("escapeChar_singleQuote"), apply("escapeChar_backspace"), apply("escapeChar_lineFeed"),
						apply("escapeChar_carriageReturn"), apply("escapeChar_tab"),
						apply("escapeChar_unicodeCodePoint"), apply("escapeChar_unicodeEscape"),
						apply("escapeChar_hexEscape")));
		...
		grammar.newRule("escapeChar_unicodeCodePoint")
				.body(seq(terminal("\\u{"), apply("hexDigit"), opt(apply("hexDigit")), opt(apply("hexDigit")),
						opt(apply("hexDigit")), opt(apply("hexDigit")), opt(apply("hexDigit")), terminal("}")));
		grammar.newRule("escapeChar_unicodeEscape")
				.body(seq(terminal("\\u"), apply("hexDigit"), apply("hexDigit"), apply("hexDigit"), apply("hexDigit")));
		grammar.newRule("escapeChar_hexEscape").body(seq(terminal("\\x"), apply("hexDigit"), apply("hexDigit")));
		...
		grammar.newRule("comment_singleLine").body(
				seq(terminal("//"), star(seq(not(terminal("\n")), any())), lookahead(alt(terminal("\n"), end()))));
		...
		grammar.newRule("token").body(alt(apply("caseName"), apply("comment"), apply("ident"), apply("operator"),
				apply("punctuation"), apply("terminal"), apply("any")));
		grammar.newRule("operator").body(alt(terminal("<:"), terminal("="), terminal(":="), terminal("+="),
				terminal("*"), terminal("+"), terminal("?"), terminal("~"), terminal("&")));
		grammar.newRule("punctuation").body(alt(terminal("<"), terminal(">"), terminal(","), terminal("--")));
	...
	public <T extends Semantics> SemanticsBlueprint<T> createSemanticsBlueprint(Class<T> semanticsClass) {
