...
/**
 * A Operation represents a function to be applied to a concrete syntax tree (CST) -- it's very
...
public class Operation {
   ...
   * Returns either
   * <ul>
   * <li>the name given to {@code myClass} via an {@code @Named} annotation (e.g. {@code "foobar"}
   * for {@code @Named("foobar")},</li>
  ...
  Object executeAction(SemanticAction action, Node node) {
    ...
    try {
      ...
      throw new OhmException(
          "There was an error executing the action '%s'.".formatted(action.name));
    } finally {
      self = previousSelf;
    }
