...
public class NodeWrapper implements Node {
	...
	/**
	 * The interval that the childOffsets of `node` are relative to. It should be
	 * the source of the closest Nonterminal node.
	 */
	...
	protected NodeWrapper wrap(ParseNode node, SourceInterval sourceInterval, SourceInterval baseInterval) {
	...
	/**
	 * Returns the wrapper of the specified child node. Child wrappers are created
	 * lazily and cached in the parent wrapper's `wrappedChildren` instance
	 * variable.
	 */
	@Override
	...
	/**
	 * Returns an array containing the wrappers of all of the children of the node
