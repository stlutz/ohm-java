			...
			for (int i = 0; i < grammarsToBuild.size(); i++) {
				...
				if (namespace.has(gDef.name)) {
					...
					if (namespace.get(gDef.name).isBuiltIn()) {
						...
						throw new OhmException("The grammar '%s' was already declared in the provided namespace."
								.formatted(gDef.name));
					}
				}
	...
	private Rule buildRule(RuleDefinition def, Grammar superGrammar) {
		...
		if (def.operation != Rule.Operation.DEFINE) {
			...
			if (superRule == null) {
				if (superGrammar == null) {
					throw new OhmException(
							"Cannot %s rule '%s'. No super grammar was specified.".formatted("override", def.name));
				} else {
					throw new OhmException("Cannot %s rule '%s'. No rule of the same name was found in a super grammar."
							.formatted("override", def.name));
				}
			}
			String[] superRuleFormals = superRule.getFormals();
			if (superRuleFormals.length != def.formals.length) {
				throw new OhmException("Cannot %s rule '%s': Got %d parameters, but super rule has %d parameters."
