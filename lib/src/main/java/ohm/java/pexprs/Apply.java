...
public class Apply extends PExpr {
	...
	private boolean handleCycle(MatchState matchState, InputStream inputStream) {
		...
		if (currentLeftRecursion != null && currentLeftRecursion.getHeadApplication().toMemoKey().equals(memoKey)) {
	...
	private boolean reallyEval(MatchState matchState, InputStream inputStream, int originalPosition) {
		...
		boolean isHeadOfLeftRecursion = (currentLR != null)
				&& (currentLR.getHeadApplication().toMemoKey().equals(memoKey));

		MemoizationRecord memoRec;
		if (isHeadOfLeftRecursion) {
			nodeOrNull = growSeedResult(rule.getBody(), matchState, originalPosition, currentLR, nodeOrNull);
