...
class InputStreamTest {
	...
	// 1F3FD -> D83C DFFD
	// 1F3FB -> D83C DFFB
	private static final String unicodeString = "Good job \uD83D\uDC4D \uD83D\uDC4D\uD83C\uDFFF \uD83D\uDC4D\uD83C\uDFFD \uD83D\uDC4D\uD83C\uDFFB!!!";
	...
	void testNextCharCode() {
		...
		assertThrows(RuntimeException.class, () -> stream.nextCharCode(), "Should throw exception if at end");
	}
	...
	void testNextCodePoint() {
		...
		assertThrows(RuntimeException.class, () -> stream.nextCodePoint(), "Should throw exception if at end");
	}
	...
	void testUnicodeNextCodePoint() {
		...
		assertEquals(0xDC4D, stream.nextCodePoint(), "Should return char code if in the middle of a code point");
	...
	void testMatchesStringCaseInsensitive() {
		...
		assertFalse(stream.matches("dog. ", true), "Should not error or match when matching past the end");
	}
